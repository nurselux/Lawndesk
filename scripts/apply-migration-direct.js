...
/**
 * Apply SMS consent migration using REST API
 * Usage:
...
console.log('🚀 Starting SMS consent migration (Direct REST API)...')
...
if (!supabaseUrl || !supabaseAnonKey) {
  ...
  console.log('3. Paste and run the migration SQL from supabase/migrations/20260402000001_sms_consent_column.sql')
...
// Execute via REST API (fallback method)
...
const curl = spawn('curl', [
  ...
  '-d', `{
    "query": "SELECT exec(${migrationSQL.replace(/\n/g, ' ')})"
  }`
])
...
curl.stdout.on('data', (data) => {
  ...
  if (response.error) {
    ...
    console.log('Result:', response.result || JSON.stringify(response.data))
  }
})
...
curl.on('close', (code) => {
  ...
  if (code === 0) {
    ...
    console.log('\n🎉 Next steps:')
    console.log(' 1. Your signup page at /signup is now live with SMS consent checkbox')
